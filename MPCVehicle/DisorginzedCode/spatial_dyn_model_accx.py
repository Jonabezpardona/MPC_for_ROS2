...
def CreateOcpSolver_SpatialDyn() -> AcadosOcp:
    ...
    ocp.model.cost_y_expr = vertcat(model.x[0], model.x[1],model.x[4], model.u) #
  
    #terminal costs
    ocp.cost.cost_type_e = "NONLINEAR_LS"
    Q_mat = np.diag([1e0,5*1e2])
    ocp.cost.W_e = Q_mat*ds_ocp
    yref_e = np.array([0.0, 0.0]) 
    ocp.cost.yref_e = yref_e
    ocp.model.cost_y_expr_e = vertcat(model.x[0], model.x[1]) 
    ...
    ocp.constraints.lbx = np.array([ -np.deg2rad(25), -0.5])
    ocp.constraints.ubx = np.array([ np.deg2rad(25), 0.5])
...
def ClosedLoopSimulation():
    ...
    for i in range(Nsim): 
        ...
        if (s_sim- s_prev >= ds_ocp):
            k = np.argmin(np.abs(s_ref[:-N_horizon] - s_sim))
            for j in range(N_horizon):
                acados_ocp_solver.set(j, "yref", np.concatenate((y_ref[:2,j+k],[y_ref[4,j+k]], [0.0, 0.0])))
...
def plot_trajectory(simX, simU, y_ref):
    ...
    plt.plot(y_ref[2,:N+1], y_ref[3,:N+1], '--', alpha=0.9, label=r'Reference', color="C1", linewidth=1.5)
    plt.xlabel(r'$x [m]$')
    plt.ylabel(r'$y[m]$')
    #plt.axis('equal')
    ...
    for i in range(2):
        ...
        ax1[i].plot(timestampsy, y_ref[i,:N+1], '--', label = r'Reference', color="C1", linewidth=1.5)
    ...
    for j, idx in enumerate(state_indices):
        ...
        ax2[j].plot(timestampsy, y_ref[idx,:N+1], '--', label = r'Reference',   color="#ffd166", linewidth=1.5)

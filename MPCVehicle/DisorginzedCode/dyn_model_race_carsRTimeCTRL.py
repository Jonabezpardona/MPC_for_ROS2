...
ds_ocp = 0.001
dt_sim = 0.001
dt_control = 0.01 # 100 Hz control frequency
Tf =N_horizon * ds_ocp
...
def CreateSimSolver_Time() -> AcadosOcp:
    ...
    #terminal cost
    ocp.cost.cost_type_e = "NONLINEAR_LS"
    ocp.cost.W_e =Q_mat * dt_sim#2 * np.diag([5*1e1, 5*1e1, 1e0, 1e0, 5*1e1,1e1]) *dt_ocp
...
def ClosedLoopSimulation():
    ...
    # Initialization
    xcurrent = y_ref[0, 2:]  # time-domain state [x, y, vx, vy, ψ, ω]

...
def create_ocp_solver_WorldFrame() -> AcadosOcp:
    ...
    Q_mat = 2 * np.diag([5*1e2, 5*1e3, 1e1, 1e0, 5*1e1,1e2,1e-1,1e0])  
    ...
    # set constraints                                                                                                               
    ocp.constraints.lbu = np.array([-np.deg2rad(20),-0.2])#-np.deg2rad(45)
    ocp.constraints.ubu = np.array([np.deg2rad(20), 0.2])#np.deg2rad(45)
...
def create_ocp_solver_BodyFrame() -> AcadosOcp:
    ...
    #terminal cost
    ocp.cost.cost_type_e = "NONLINEAR_LS"
    ocp.cost.W_e =2 * np.diag([5*1e1, 5*1e0, 1e1, 1e1, 5*1e1,1e1]) *dt_ocp
    yref_e = np.array([10, 0.0, 0.0, 0.0, np.pi/2, 0.0]) 
    ocp.cost.yref_e = yref_e
    ocp.model.cost_y_expr_e = model.x #vertcat(model.x[:4]) 

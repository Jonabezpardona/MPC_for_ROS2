...
lf = 2.3 # distance from CoG to front wheels
lr = 1.2 #distance form CoG to rear wheels
L = lr + lf # wheel base
    ...
    for i in range(N): 
        ...
        if (s_sim- s_prev >= ds_ocp) or(s_sim > s_ref[k+1]):
            for j in range(N_horizon):
                acados_ocp_solver.set(j, "yref", np.concatenate((y_ref[j+k,:2], [y_ref[j+k,-1]],[0.0, 0.1])))
                acados_ocp_solver.set(j, "p", np.array([s_ref[k + j]]))
            acados_ocp_solver.set(N_horizon, "yref",np.concatenate((y_ref[k+N_horizon,:2], [y_ref[N_horizon+k,-1]])) )
        ...
        xcurrent = acados_sim_solver.simulate(xcurrent, u0, s_ref[k+1]) 
        simU[i, :] = u0
...
def plot_trajectory(simX, simU, y_ref, Beta):
    ...
    labels = ["e_psi", "e_y", "x", "y", "theta", "v", "a", "delta", "beta"]
    for i in range(nx):

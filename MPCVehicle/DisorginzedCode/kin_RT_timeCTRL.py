...
# Add at the top with your other timing parameters:
dt_sim = 0.01       # simulation step (already present)
dt_control = 0.01    # control period (set 0.01 for 100 Hz)  <-- new
...
S, s_0, s_ref, kappa_ref, y_ref = ellipse.spatial_offline_ref(ds_ocp, N_horizon)
...
def create_sim_solver() -> AcadosOcp:
    ...
    ny_e = nx
    ocp.solver_options.N_horizon = N_horizon
    Q_mat = 2 * np.diag([1e1,1e1, 1e1, 1e2])  #ellipse
...
def closed_loop_simulation():
    ...
    #AcadosIntegrator (time-domain sim)
    ...
    # initialization
    xcurrent = y_ref[0, 2:].copy()        # time-domain state: [x, y, psi, v] (initial)
    xcurr_ocp = X0.copy()                 # spatial-state for ocp (initial guess)
    predX[0,:] = xcurr_ocp
    simX[0, :] = xcurr_ocp
    s_sim = s_0[0]
    ...
    # set initial solver states & parameters
    for stage in range(N_horizon + 1):
        acados_ocp_solver.set(stage, "x", xcurr_ocp)
        # safe default param = s_ref[stage] (will be overwritten at runtime)
    ...
    # variables for control scheduling
    control_timer = 0.0
    u0 = np.zeros((nu,))  # initial control
    i = 0

    # main loop
    while simulation_running:
        # ---- CONTROL (periodic) ----
        # compute whether it is time to run the controller based on current time-domain state xcurrent
        # NOTE: we compute spatial s from the **current** time-state (before applying simulation)
        if control_timer >= dt_control - 1e-12:
            # compute current s, epsi, ey from time state
            s_now, epsi_now, ey_now = time2spatial(xcurrent[0], xcurrent[1], xcurrent[2], s_ref, y_ref[:,2:5])

            # find closest reference index
            idx_closest = int(np.argmin(np.abs(s_ref - s_now)))

            # Build horizon starting from closest ref index (handle end bounds)
            for j in range(N_horizon):
                idx_stage = min(idx_closest + j, len(s_ref) - 1)
                # set reference yref for stage (first 2 elements are epsi, ey)
                acados_ocp_solver.set(j, "yref", np.concatenate((y_ref[idx_stage, :2], [0.0, 0.0])))
                # set parameter p = s_ref for that stage
            ...
            # lock initial state of OCP to current measured spatial state
            x0_ocp = np.hstack((epsi_now, ey_now, xcurrent))  # [e_psi, e_y, x, y, psi, v]
            acados_ocp_solver.set(0, "lbx", x0_ocp)
            acados_ocp_solver.set(0, "ubx", x0_ocp)
            # optional: set stage 0 initial guess too
            ...
            # retrieve predicted trajectories (optional bookkeeping)
            try:
                for j in range(N_horizon):
                ...
                # If index issues etc., ignore predictions (they are optional)
                pass

            # get first input
            u0 = acados_ocp_solver.get(0, "u").copy()

            # reset control timer (support catching-up if control_timer >> dt_control)
            control_timer = control_timer - dt_control

        # ---- SIMULATION STEP ----
        simU[i, :] = u0
        # simulate one dt_sim forward using u0
        x_next = acados_sim_solver.simulate(xcurrent, u0)
        xcurrent = x_next.copy()

        # Convert to spatial and store simulation logs
        s_sim_new, epsi_sim, ey_sim = time2spatial(xcurrent[0], xcurrent[1], xcurrent[2], s_ref, y_ref[:,2:5])
        simX[i + 1, :] = np.hstack((epsi_sim, ey_sim, xcurrent))
        # keep representation consistent with your previous code (epsi + beta)
        simX[i + 1, 0] = simX[i + 1, 0] + arctan(lr * tan(simU[i, 1]) / L)
        simX[i + 1, -2] = simX[i + 1, -2] + arctan(lr * tan(simU[i, 1]) / L)
        S_sim = np.append(S_sim, s_sim_new)

        # update ocp initial guess for next iteration
        xcurr_ocp = np.hstack((epsi_sim, ey_sim, xcurrent))
        # restore lb/ub on stage 0 to current state (so next ctrl solve can use it)
        acados_ocp_solver.set(0, "lbx", xcurr_ocp)
        acados_ocp_solver.set(0, "ubx", xcurr_ocp)

        # prints / logging
        print('Ssim (current):', s_sim_new)
        ...
        # termination
        if s_sim_new >= (S - 2*ds_ocp) or i >= Nsim:
            simulation_running = False

    # prepare plots as before

...
def ClosedLoopSimulation():
    # Setup solvers
    ocp = spatial_solver.Solver_SpatialDyn_LinVL(d_ocp,a_max)
    acados_ocp_solver = AcadosOcpSolver(ocp, json_file='acados_ocp_spatial1.json')

    sim_solver =time_solver.Solver_TimeDyn_LinVL(d_sim, a_max)
    sim = AcadosSim()
    ...
    # Initialization
    y_ref = y_ref_dyn  
    xcurrent = y_ref[2:, 0 ]  # time-domain state [x, y, vx, vy, ψ, ω, Fz]
    print(xcurrent)
    xcurr_ocp = X0.copy()    # spatial-domain state [eψ, ey, x, y, vx, vy, ψ, ω, Fz]
...
def plot_states(simX, simU, y_ref):
    ...
    # --- 2. States ---
    state_indices = [2, 3, 4, 5, 6, 7, 8]  # x, y, vx, vy, theta, omega, Fz
    state_labels = [r'$x[m]$', r'$y[m]$', r'$v_x[\frac{m}{s}]$', r'$v_y[\frac{m}{s}]$', r'$\theta[rad]$', r'$\omega[\frac{rad}{s}]$', r'$\Delta F_z[]$']

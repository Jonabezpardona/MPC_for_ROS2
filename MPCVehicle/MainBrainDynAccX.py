...
from Animation import SimulationAnimator # if we want to animate, uncomment the lines 
...
y_ref_kin, y_ref_dyn, S, s_ref, kappa_ref= ellipse.Spatial_Ref(omega, d_ocp, N_horizon) # depending on the model we choose the reference
X0 = y_ref_dyn[:-1,0]
print(X0)
print(y_ref_dyn.shape)
...
anim = SimulationAnimator( y_ref_dyn[2,:], y_ref_dyn[3,:],y_ref_dyn[6,:] )

# ------------------ CLOSED LOOP SIMULATION------------------

def ClosedLoopSimulation():
    # Setup solvers
    ocp = spatial_solver.Solver_SpatialDyn_Accx(d_ocp)
    acados_ocp_solver = AcadosOcpSolver(ocp, json_file='acados_ocp_spatial1.json')

    sim_solver =time_solver.Solver_TimeDyn_Accx(d_sim)
    sim = AcadosSim()
    ...
    # Initialization
    y_ref = y_ref_dyn[:-1,:]  # this is since you are not using deltaFz
    xcurrent = y_ref[2:, 0 ]  # time-domain state [x, y, vx, vy, ψ, ω]
    print(xcurrent)
    ...
    while simulation_running:
        ...
        if control_timer >= dt_control - 1e-9:
            ...
            # Update horizon referencess
            for j in range(N_horizon):
                idx_stage = min(idx_closest + j, len(s_ref) - 1)
                yref_stage = np.concatenate(
                    (y_ref[:2, idx_stage], [y_ref[4, idx_stage]], [0.0, 0.0])
                )
        ...
        x_logged[0] += np.arctan(x_logged[5] / x_logged[4])
        x_logged[-2] += np.arctan(x_logged[5] / x_logged[4])
    ...
    anim.animate(simX[:i+1, 2],simX[:i+1, 3],simX[:i+1, 4],simX[:i+1, 6],simU[:i+1, 1],t)

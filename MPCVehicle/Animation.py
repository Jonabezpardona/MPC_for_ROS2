...
class SimulationAnimator:
    ...
    def __init__(self, x_ref, y_ref,psi, lf= 0.13, lr = 0.13,car_width = 0.17, wheel_length=0.03, wheel_width=0.02):
        self.x_data = []
        self.y_data = []
    ...
    def update_anim(self, frame, simX, simY,simV,simP,simD, times):
        ...
        # keep camera centered - just for a better view of the car
...
if __name__ == "__main__":
    ...
    for i in range(len(t_ref)):
        # in Reality here we use MPC ans simulation for x, y, psi, delta
        px = x_ref[i]
        py = y_ref[i]
        psi = np.arctan2(np.gradient(y_ref)[i], np.gradient(x_ref)[i])  # approx orientation 
